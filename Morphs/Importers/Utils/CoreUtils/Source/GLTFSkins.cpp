...
void GLTFSkins::LoadSkinData(const Microsoft::glTF::Document& gltfDocument, const Microsoft::glTF::GLTFResourceReader& reader)
{
    if (!m_skins.empty())
    {
        throw Babylon::Utils::BabylonException("GLTFSkins::LoadSkinData() -- skins have already been loaded");
    }
    ...
    for (auto& skin : gltfDocument.skins.Elements())
    {
        ...
        if (!skin.inverseBindMatricesAccessorId.empty())
        {
            ...
            {
                throw Babylon::Utils::BabylonException("GLTFSkins::LoadSkinData() -- invalid size for inverse bind matrix data");
            }

            size_t numIbms = inverseBindMatricesData.size() / 16;
            BabylonAssert(sizeof(Microsoft::glTF::Matrix4) == sizeof(float) * 16);
...
void GLTFSkins::RetargetNodeIds(const std::map<std::string /*oldNodeId*/, std::string/*newNodeId*/>& oldNodeToNewNodeIdMap)
{
    ...
    for (auto& skin : m_skins)
    {
        for (auto& jointNodeId : skin.second.skin.jointIds)
        {
            ...
            {
                throw Babylon::Utils::BabylonInvalidArgException("GLTFSkins::RetargetNodeIds() -- skin joint node does not have a remapping entry");
            }
        }
        ...
        if (!skin.second.skin.skeletonId.empty())
        {
            ...
            {
                throw Babylon::Utils::BabylonInvalidArgException("GLTFSkins::RetargetNodeIds() -- skin skeletonId node does not have a remapping entry");
            }
        }
    }
}
