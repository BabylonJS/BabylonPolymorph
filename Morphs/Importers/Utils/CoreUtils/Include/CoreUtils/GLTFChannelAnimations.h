...
namespace GLTFChannelInterpolators
{
    ...
    {
        BabylonAssert(0.0f <= t && t <= 1.0f);
        BabylonUnusedParameter(t);
        return Normalize(q0);
    }
...
public:
    ...
    Babylon::Utils::glTF::AnimationMetadata::eType GetAnimationTypes(const std::string& animationId, const std::set<std::string>& jointNodeIds) const;
...
protected:
    struct SamplerChannelData
    {
            ...
            numScales(scale.size() / 3)
        {
            ...
            {
                throw Babylon::Utils::BabylonInvalidArgException("SamplerChannelData -- invalid number of position components on channel");
            }

            if (rotation.size() % 4 != 0)
            {
                throw Babylon::Utils::BabylonInvalidArgException("SamplerChannelData -- invalid number of rotation components on channel");
            }

            if (scale.size() % 3 != 0)
            {
                throw Babylon::Utils::BabylonInvalidArgException("SamplerChannelData -- invalid number of scale components on channel");
            }
        }
    ...
    const std::vector<Microsoft::glTF::Vector3>& outputValues)
{
    ...
    case Microsoft::glTF::InterpolationType::INTERPOLATION_UNKNOWN:
        throw Babylon::Utils::BabylonInvalidArgException("Interpolate() -- invalid interpolation type UNKNOWN specified");
    }
    ...
    const uint32_t dim)
{
    ...
    case Microsoft::glTF::InterpolationType::INTERPOLATION_CUBICSPLINE:
        ...
        break;
    case Microsoft::glTF::InterpolationType::INTERPOLATION_UNKNOWN:
        throw Babylon::Utils::BabylonInvalidArgException("InterpolateWeights() -- invalid interpolation type UNKNOWN specified");
    }
    ...
    Microsoft::glTF::Vector3& result)
{
    ...
    if (animationIt != m_animations.end())
    {
        ...
        for (const auto& channel : channels.Elements())
        {
            if (channel.target.path == targetPath && channel.target.nodeId == nodeId)
            {
                ...
                {
                    throw Babylon::Utils::BabylonInvalidArgException("Evaluate() -- only TRANSLATION and SCALE target types are supported for the Vector3 specialization");
                }
    ...
    {
        throw Babylon::Utils::BabylonInvalidArgException("Evaluate() -- invalid animationId specified");
    }
    ...
    if (animationIt != m_animations.end())
    {
        ...
        for (const auto& channel : channels.Elements())
        {
            if (channel.target.path == targetPath && channel.target.nodeId == nodeId)
            {
                ...
                {
                    throw Babylon::Utils::BabylonInvalidArgException("Evaluate() -- only ROTATION target type is supported for the Quaternion specialization");
                }
    ...
    if (animationIt != m_animations.end())
    {
        ...
        for (const auto& channel : channels.Elements())
        {
            if (channel.target.path == targetPath && channel.target.nodeId == nodeId)
            {
                ...
                {
                    throw Babylon::Utils::BabylonInvalidArgException("Evaluate() -- only WEIGHTS target type is supported for the std::vector<float> specialization");
                }
