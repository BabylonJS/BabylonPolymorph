...
namespace Babylon
{
    namespace Framework
    {
        ...
        {
            enum BabylonRenderingMode : uint8_t
            {
                ...
                MaskBlend           = 4,        // "combined mask and blend" mode.
                Add                 = 5,        // Reserved for future use
                Multiply            = 6,        // Reserved for future use
            };

            using BabylonRenderingOptions = uint16_t;

            enum BabylonRenderingOption : BabylonRenderingOptions
            {
            ...
            void SetBabylonRenderingMode(BabylonRenderingMode mode)
            {
                SetBabylonRenderingMode(ShaderType, mode);
            }

            void AppendBabylonRenderingOption(BabylonRenderingOption option)
            {
                AppendBabylonRenderingOption(ShaderType, option);
            }

            void SetBabylonRenderingOptions(BabylonRenderingOptions options)
            {
            ...
            BabylonRenderingMode GetBabylonRenderingMode() const
            {
                return GetBabylonRenderingMode(ShaderType);
            }

            BabylonRenderingOptions GetBabylonRenderingOptions() const
            {
                return static_cast<BabylonRenderingOptions>(ShaderType & 0b00000000'00000000'11111111'11111111);
            }

            bool HasBabylonRenderingOption(BabylonRenderingOption option) const
            {
                return TestBabylonRenderingOption(ShaderType, option);
            }

            static void AppendBabylonRenderingOption(uint32_t& shaderType, BabylonRenderingOption option)
            {
                shaderType |= static_cast<uint32_t>(option);
            }

            static void SetBabylonRenderingMode(uint32_t& shaderType, BabylonRenderingMode mode)
            {
            ...
            static bool TestBabylonRenderingOption(uint32_t shaderType, BabylonRenderingOption option)
            {
                return (GetBabylonRenderingOptions(shaderType) & option) == option;
            }

            static BabylonRenderingMode GetBabylonRenderingMode(uint32_t shaderType)
            {
                auto shifted = static_cast<uint8_t>(shaderType >> 2 * 8);
                return static_cast<BabylonRenderingMode>(shifted);
            }

            static BabylonRenderingOptions GetBabylonRenderingOptions(uint32_t shaderType)
            {
                return static_cast<BabylonRenderingOptions>(shaderType);            // Just return the lower 16 bits
            }
        ...
        {
            using EFilterMode = Babylon::Framework::FilterMode;
            using EAddressMode = Babylon::Framework::AddressMode;
