...
using SpecularGlossinessValue = Microsoft::glTF::SpecularGlossinessValueTypeless<Babylon::Utils::Math::Color>;
using MetallicRoughnessValue  = Microsoft::glTF::MetallicRoughnessValueTypeless<Babylon::Utils::Math::Color>;
...
namespace
{
    // Use the glTF SDK ToGamma function rather than Babylon::Utils::Math::Color::ToGammaRGB to
    ...
    // Use the glTF SDK ToLinear function rather than Babylon::Utils::Math::Color::ToLinearRGB to
    // match existing Transcoders behaviour (glTF SDK has a more accurate colour space conversion)
    ...
    {
    public:
        Babylon::Utils::Math::Color operator()(uint32_t, uint32_t) const
        {
    ...
    {
    public:
        ConverterTextureAccessorSRGB(Babylon::Transcoder::BGRAImage&& image) :
            m_image(std::move(image))
        {
        }

        Babylon::Utils::Math::Color operator()(uint32_t x, uint32_t y) const
        {
    ...
    {
    public:
        ConverterTextureAccessorLinear(Babylon::Transcoder::BGRAImage&& image) :
            m_image(std::move(image))
        {
        }
    ...
    ConvertedMaterialData ConvertMaterialSGToMR(const SpecularGlossinessValue& sg, const uint32_t textureWidth, const uint32_t textureHeight, FnDiffuse fnDiffuse, FnSpecular fnSpecular)
    {
        ...
        for (uint32_t y = 0U; y < textureHeight; ++y)
        {
            for (uint32_t x = 0U; x < textureWidth; ++x)
            {
                ...
                const auto mrValue = Microsoft::glTF::SGToMR<Babylon::Utils::Math::Color>(sgCopy);

                {
                    const Babylon::Utils::Math::Color baseColour(ToGamma(mrValue.base.R()), ToGamma(mrValue.base.G()), ToGamma(mrValue.base.B()), mrValue.opacity);
                    const uint32_t value = baseColour.BGRA();
                    memcpy_s(generatedData.m_dataAlbedo.get() + index, pixelBytes - index, &value, sizeof(value));
                }

                {
                    const Babylon::Utils::Math::Color metallicRoughness(0.0f, mrValue.roughness, mrValue.metallic, 1.0f);
    ...
    ConvertedMaterialData ConvertMaterialMRToSG(const MetallicRoughnessValue& mr, const uint32_t textureWidth, const uint32_t textureHeight, FnBaseColor fnBaseColor, FnMetalRough fnMetalRough)
    {
        ...
        for (uint32_t y = 0U; y < textureHeight; ++y)
        {
            for (uint32_t x = 0U; x < textureWidth; ++x)
            {
                ...
                const auto sgValue = Microsoft::glTF::MRToSG<Babylon::Utils::Math::Color>(mrCopy);

                {
                    const Babylon::Utils::Math::Color diffuse(ToGamma(sgValue.diffuse.R()), ToGamma(sgValue.diffuse.G()), ToGamma(sgValue.diffuse.B()), sgValue.opacity);
                    const uint32_t value = diffuse.BGRA();
                    memcpy_s(generatedData.m_dataAlbedo.get() + index, pixelBytes - index, &value, sizeof(value));
                }

                {
                    const Babylon::Utils::Math::Color specGloss(ToGamma(sgValue.specular.R()), ToGamma(sgValue.specular.G()), ToGamma(sgValue.specular.B()), sgValue.glossiness);
    ...
    )
    {
        ...
        return TexturePair(
            std::make_shared<TextureDescriptor>(std::move(materialData.m_dataAlbedo),
                materialData.m_dataSize, width, height, width * 4U, Babylon::Framework::TextureSegment::EFormat::kFormatBGRA32),
...
AlbedoWorkflowPair MaterialConversionUtils::SpecGlossToMetalRough(const std::shared_ptr<const MaterialLayer>& diffuseLayer, const std::shared_ptr<const MaterialLayer>& specGlossLayer)
{
    ...
    {
        auto mrFactors = Microsoft::glTF::SGToMR<Babylon::Utils::Math::Color>(sgFactors);
