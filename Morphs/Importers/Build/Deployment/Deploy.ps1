...
function DeployChangelog($imageDirectory)
{
    ...
    Get-ChildItem -Path "$spectreRoot\Build\Logs\*" -Include *.md | `
...
{
    # Find the project's NuGet package definition
    Get-ChildItem -Path "$spectreRoot\Build\NuGet\*" -Include *.nuspec, *.autopkg, *.targets, *.props, *.ps1 | `
...
{
    cd "$spectreRoot\Build\Documentation"
...
{
    $pluginOutDir = "$spectreRoot\Built\Out\$compiler\$platform\$configuration\$projectName"
    $pluginImageDir = "$spectreRoot\Built\Image\Plugins\$platform\$configuration"
...
function ProcessProjectDeploymentDefinition($includeDirectory, $project, $libraryDirectory)
{
    ...
    {
        $outputDirectory = "$spectreRoot\Built\Out\$compiler\$platform\$configuration\$innerProjectName"
    ...
    $projectPath = "$spectreRoot\$projectLocation\$innerProjectName\cpp\Inc"
...
{
    $projectToDeploy     = $file.SpectreProjectDeployment.name
    $deployDocumentation = $file.SpectreProjectDeployment.documentation
    $packageType         = $file.SpectreProjectDeployment.package
    $imageDirectory      = "$spectreRoot\Built\Image\$projectToDeploy"
    $outputDirectory     = "$spectreRoot\Built\Out\$compiler\$platform\$configuration\$projectToDeploy"
    ...
    # Copy include files
    $projects = $file.SpectreProjectDeployment.Project
...
{
    cd "$spectreRoot\Build\Deployment"
...
{
    DeployProject "$spectreRoot" $projectName $compiler $platform $configuration $clean
}
elseif ($type -eq [string]"plugin")
{
    DeployPlugin "$spectreRoot" $projectName $compiler $platform $configuration
}
else
{
