...
namespace
{
    ...
    {
    #if _DEBUG
        return Spectre::Utils::IO::GetTempDirectory() + "SavedHudSettings.txt";
    #else
...
{
    SpectreAssert(g_TraceDisabledCount > 0);
...
{
    SpectreAssert(allLevels[level].level == level);
...
void SaveCurrentTraceSettings()
{
    ...
    try
    {
        Spectre::Utils::IO::DebugOnlyWriteWholeFile(configFileName, contents);
    }
    catch (std::bad_alloc&)
    {
        throw;
...
void LoadCurrentTraceSettings()
{
    ...
    try
    {
        std::string contents = Spectre::Utils::IO::DebugOnlyReadWholeBlockingFileAsString(configFileName);
        std::vector<std::string> allLines = Spectre::Utils::Split(contents, "\n");
        ...
        {
            auto pair = Spectre::Utils::Split(line, "=");
            if (pair.size() != 2)
            {
...
{
    SpectreAssert((device >= 0) && (device < Device::MaxDevice));
...
{
    this->ResetLevels();
    auto items = Spectre::Utils::Split(line, ";");
    for (auto& item : items)
    {
        auto pair = Spectre::Utils::Split(item, ":");
        if (pair.size() != 2)
        {
            throw Spectre::Utils::SpectreException("Wrong line format of " + line);
        }
        auto& key = pair[0];
        auto& val = pair[1];
        ...
        {
            throw Spectre::Utils::SpectreException("Bad line description '" + key + "'");
        }
    }
}
