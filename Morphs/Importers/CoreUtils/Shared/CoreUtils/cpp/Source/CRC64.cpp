...
namespace Spectre
{
    namespace Utils
    {
        namespace CRC64
        {
            const uint64_t k_CRC64Table[256] ={
            ...
            {
                return static_cast<Checksum>(k_CRC64Table[static_cast<uint8_t>(crc) ^ byte] ^ (static_cast<uint64_t>(crc) >> 8));
            }
            ...
            {
                crc = HashByte(crc, static_cast<uint8_t>(bytes));
                crc = HashByte(crc, static_cast<uint8_t>(bytes >> 8));
            ...
            Checksum Hash4Bytes(Checksum crc, uint32_t bytes)
            {
                crc = HashByte(crc, static_cast<uint8_t>(bytes));
                crc = HashByte(crc, static_cast<uint8_t>(bytes >>  8));
                crc = HashByte(crc, static_cast<uint8_t>(bytes >> 16));
                crc = HashByte(crc, static_cast<uint8_t>(bytes >> 24));
