...
namespace
{
    ...
    void ValidateKeyFrame(Microsoft::glTF::InterpolationType interpolation, size_t keyFrameTimeCount, size_t keyFrameValueCount)
    {
        ...
        {
            throw Spectre::Utils::SpectreException("GLTFChannelAnimations::ValidateKeyFrame() -- the number of keyframe animation times does not match keyframe values");
        }
    }
}
...
void GLTFChannelAnimations::ValidateAccessorData() const
{
    for (auto& animationPair : m_animations)
    {
        ...
        for (auto& channel : gltfAnimation.channels.Elements())
        {
            ...
            {
                throw Spectre::Utils::SpectreException((std::string("GLTFChannelAnimations::ValidateAccessorData() -- animation samplerId ") + channel.samplerId + " not found").c_str());
            }
            ...
            {
                throw Spectre::Utils::SpectreException("GLTFChannelAnimations::ValidateAccessorData() -- input accessor not found");
            }

            auto outputAccessorIt = m_outputAccessors.find(sampler.outputAccessorId);
            if (outputAccessorIt == m_outputAccessors.end())
            {
                throw Spectre::Utils::SpectreException("GLTFChannelAnimations::ValidateAccessorData() -- output accessor not found");
            }
...
void GLTFChannelAnimations::LoadAnimationData(const std::string& gltfAnimationId, const Microsoft::glTF::Document& gltfDocument, const Microsoft::glTF::GLTFResourceReader& reader)
{
    ...
    if (!it.second)
    {
        throw Spectre::Utils::SpectreInvalidArgException("GLTFChannelAnimations::LoadAnimationData() -- animationId already loaded");
    }
    ...
    {
        throw Spectre::Utils::SpectreException("GLTFChannelAnimations::LoadAnimationData() -- number of animations in document does not match previous value");
    }

    size_t animationIndex = gltfDocument.animations.GetIndex(gltfAnimationId);
    if (!m_animationOrdering[animationIndex].empty())
    {
        throw Spectre::Utils::SpectreInvalidArgException("GLTFChannelAnimations::LoadAnimationData() -- invalid animationId specified");
    }
...
std::vector<std::string> GLTFChannelAnimations::GetTargetedNodeIds(const std::string& animationId) const
{
    ...
    {
        throw Spectre::Utils::SpectreInvalidArgException("GLTFChannelAnimations::GetTargetedNodeIds() -- invalid animationId specified");
    }
    ...
    for (auto& animationPair : m_animations)
    {
        for (const auto& channel : animationPair.second.channels.Elements())
        {
            if (!channel.target.nodeId.empty())
            {
                ...
                {
                    throw Spectre::Utils::SpectreInvalidArgException("GLTFChannelAnimations::RetargetNodeIds() -- channel target node does not have a remapping entry");
                }
            }
        }
    }
...
Microsoft::glTF::Animation GLTFChannelAnimations::ExportAnimationData(size_t animationIndex, Microsoft::glTF::BufferBuilder& bufferBuilder) const
{
    ...
    for (auto& channel : animation.channels.Elements())
    {
        ...
        if (serializedSamplers.find(sampler.id) == serializedSamplers.end())
        {
            ...
            SpectreAssert(inputAccessorIt != m_inputAccessors.end());
            if (inputAccessorIt != m_inputAccessors.end())
            {
                ...
                SpectreAssert(outputAccessorIt != m_outputAccessors.end());
                if (outputAccessorIt != m_outputAccessors.end())
                {
                    ...
                    case Microsoft::glTF::TargetPath::TARGET_WEIGHTS:
                        ...
                        break;
                    default:
                        throw Spectre::Utils::SpectreInvalidArgException("GLTFChannelAnimations::ExportAnimationData() -- unknown channel target path");
                    }
...
Spectre::Utils::glTF::AnimationMetadata::eType GLTFChannelAnimations::GetAnimationTypes(const std::string& animationId, const std::set<std::string>& jointNodeIds) const
{
    ...
    for (size_t j = 0; j < animation.channels.Size(); j++)
    {
        ...
        {
            animationType |= Spectre::Utils::glTF::AnimationMetadata::kMorph;
        }
        else if (jointNodeIds.find(channel.target.nodeId) == jointNodeIds.end())
        {
            // Found animation targeting a non-joint node
            animationType |= Spectre::Utils::glTF::AnimationMetadata::kNode;
        }
        else
        {
            animationType |= Spectre::Utils::glTF::AnimationMetadata::kSkinned;
        }
    }

    return static_cast<Spectre::Utils::glTF::AnimationMetadata::eType>(animationType);
}
