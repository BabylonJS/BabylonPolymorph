...
std::vector<std::shared_ptr<WIN32_FIND_DATA>> GetFileSystemEntries(std::wstring const& path)
{
    ...
    {
        throw Utils::SpectreInvalidArgException(
    ...
    fileHandle = FindFirstFile(directorySearch.c_str(), &fileData);
    if (fileHandle == INVALID_HANDLE_VALUE)
    {
        dwError = GetLastError();
        throw Utils::SpectreException(dwError,
            "FindFirstFile returned INVALID_HANDLE_VALUE for " +
            Utils::LexicalCast<std::string>(path), false);
    }

    std::vector<std::shared_ptr<WIN32_FIND_DATA>> result;
    do
    {
        WIN32_FIND_DATA data = fileData;
        result.push_back(std::make_shared<WIN32_FIND_DATA>(data));
