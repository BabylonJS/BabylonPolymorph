...
namespace Utils
{
    inline bool IsAligned(const void* const ptr, size_t alignment)
    {
        ...
        {
            throw SpectreInvalidArgException("IsAligned: alignment argument value is not a power of two");
        }
    ...
    inline SPECTRE_CONSTEXPR size_t AlignUp(size_t value, size_t alignment)
    {
        // The requested alignment must be a power of two, or zero for no alignment
        SpectreAssert((alignment == 0U) || Math::IsPowerOfTwo(alignment));
