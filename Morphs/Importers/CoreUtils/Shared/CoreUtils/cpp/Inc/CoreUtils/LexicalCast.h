...
struct LexicalCastFailed : Spectre::Utils::SpectreException
{
...
{
    SpectreAssert(!stream.bad());
    if (stream.fail())
    {
        // e.g. you tried converting "Foo" to an int...
        throw Spectre::Utils::LexicalCastFailed("Failed to convert value");
    }
    if (checkEOF && !stream.eof())
    {
        // E.g. you tried converting "62 61" or "61p" to an int...
        throw Spectre::Utils::LexicalCastFailed("Failed to convert value as too much data was supplied");
    }
}
