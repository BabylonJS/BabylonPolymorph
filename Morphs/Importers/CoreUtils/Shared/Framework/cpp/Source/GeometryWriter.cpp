    ...
    namespace Framework
    {
        namespace V1
        {
            ...
            Frame* GeometryWriter::EndFrame()
            {
                if (m_meshId != 0)
                {
                    throw Utils::BabylonFailException("Missing call to EndMesh");
                }
            ...
            void GeometryWriter::BeginMesh(uint32_t meshId, uint32_t parentId, uint32_t materialId, uint32_t skeletonId)
            {
                ...
                {
                    throw Utils::BabylonFailException("GeometryWriter::BeginMesh: You must call BeginFrame first");
                }
                if (m_meshId != 0)
                {
                    throw Utils::BabylonFailException("Call to BeginMesh without a call to EndMesh");
                }
                if (meshId == 0)
                {
                    throw Utils::BabylonInvalidArgException("Invalid meshId of 0 specified");
                }
                BabylonAssertExpr(m_hasLockedData == false, "GeoemtryWriter::BeginMesh - has locked data");
            ...
            void GeometryWriter::EndMesh()
            {
                if (m_outputBuffer == nullptr)
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - NULL output buffer");
                }
                if (!IsValidMesh())
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - Need to call BeginMesh first");
                }
                if ((m_totalIndices % 3) != 0)
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - Invalid index count");
                }
                BabylonAssertExpr(m_hasLockedData == false, "GeoemtryWriter::EndMesh - has locked data");

                // Ensure data is valid
                if (m_totalVertices > 0 && m_joints.size() > 0)
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - Cannot have Joint and Vertex data on same object");
                }

                if (m_instanceType != None && m_joints.size() > 0)
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - Cannot have Joint data within an instance segment");
                }

                if (m_instanceType != None && m_totalVertices > 0)
                {
                    throw Utils::BabylonFailException("GeoemtryWriter::EndMesh - Cannot have Vertex data within an instance segment");
                }
            ...
            void GeometryWriter::AddJoint(uint32_t id, int parentJointIndex, const float* invBindMatrix, const float* transform)
            {
                ...
                {
                    throw Utils::BabylonFailException("GeometryWriter::AddJoint - Invalid parent joint index");
                }
            ...
            {
                BabylonAssertExpr(m_hasLockedData == false, "GeoemtryWriter::LockVertexData - already locked");
                BabylonAssertExpr(m_totalVertices == 0, "GeoemtryWriter::LockVertexData - lock only allowed on new meshes");
                BabylonAssertExpr(m_totalIndices == 0, "GeoemtryWriter::LockVertexData - lock only allowed on new meshes");
            ...
            {
                BabylonUnusedParameter(data);
                BabylonAssertExpr(m_hasLockedData == true, "GeoemtryWriter::UnlockVertexData - not locked");
                BabylonAssertExpr(data.m_numIndicesLocked == m_totalIndices, "GeoemtryWriter::UnlockVertexData - unexpected index count");
                BabylonAssertExpr(data.m_numVerticesLocked == m_totalVertices, "GeoemtryWriter::UnlockVertexData - unexpected vertex count");
                m_hasLockedData = false;
            }
                                                ...
                                                std::shared_ptr<Babylon::Framework::IBuffer> const&  outputBuffer)
            {
                ...
                {
                    throw Utils::BabylonInvalidArgException("Element size not supported. Add support if you need it");
                }
            ...
            {
                BabylonAssertExpr(m_hasLockedData == false, "GeoemtryWriter::AddVertex - has locked data");
                ...
                if (!IsValidMesh())
                {
                    throw Utils::BabylonFailException("Need to call BeginMesh first");
                }
                ...
                for (uint32_t i = 0; i < numVertices; ++i)
                {
                    ...
                    BabylonAssertExpr(v.HasNormal || m_buffers[kBufferkNormal].GetSizeInBytes() == 0, "GeometryWriter::AddVertex - If a vertex has a normal, they all need to!");
                    BabylonAssertExpr(v.HasTangent || m_buffers[kBufferTangent].GetSizeInBytes() == 0, "GeometryWriter::AddVertex - If a vertex has a tngent, they all need to!");
                    BabylonAssertExpr(v.HasUV0 || m_buffers[kBufferUV0].GetSizeInBytes() == 0, "GeometryWriter::AddVertex - If a vertex has UV0, they all need to!");
                    BabylonAssertExpr(v.HasUV1 || m_buffers[kBufferUV1].GetSizeInBytes() == 0, "GeometryWriter::AddVertex - If a vertex has UV1, they all need to!");
                    BabylonAssertExpr(v.HasColor || m_buffers[kBufferColor].GetSizeInBytes() == 0, "GeometryWriter::AddVertex - If a vertex has COLOR, they all need to!");
            ...
            void GeometryWriter::AddIndex(_In_reads_(numIndices) const uint32_t* pIndices, uint32_t numIndices)
            {
                ...
                {
                    throw Utils::BabylonFailException("Have both SMALL and BIG indices. Not allowed.");
                }
                ...
                if (numIndices <= 0)
                {
                    throw Utils::BabylonInvalidArgException("GeometryWriter: Index count must be multiple of 3 and greater and 0");
                }
                BabylonAssertExpr(m_hasLockedData == false, "GeoemtryWriter::AddIndex - has locked data");
        ...
        BABYLON_SDK_API IGeometryWriter* GeometryWriterCreate()
        {
            return new V1::GeometryWriter();
        }

        BABYLON_SDK_API void GeometryWriterDestroy(IGeometryWriter*& pWriter)
        {
            if (pWriter == nullptr)
                return;
