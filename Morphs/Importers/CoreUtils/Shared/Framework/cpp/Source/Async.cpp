...
namespace SF = Spectre::Framework;

//-----------------------------------------------------------------------------

AsyncActionWithProgress::AsyncActionWithProgress(void* userData) :
    m_cancellationToken(Spectre::Framework::MakeCancellationToken()),
    m_status(SpectreAsyncStatus::Started),
    m_userData(userData),
    m_currentValue(0ul)
{}
...
RegistrationToken AsyncActionWithProgress::AddProgressCallback(AsyncActionProgressHandler handler)
{
    ...
    if (m_status == SpectreAsyncStatus::Started)
    {
...
{
    SetTerminalStatus(SpectreAsyncStatus::Canceled, "Canceled", nullptr);
}
...
{
    SetTerminalStatus(SpectreAsyncStatus::Completed, "Completed", nullptr);
}
...
{
    SetTerminalStatus(SpectreAsyncStatus::Error, message, exception);
}

//-----------------------------------------------------------------------------

Spectre::Framework::ICancellationTokenPtr AsyncActionWithProgress::GetCancellationToken() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
SpectreAsyncStatus AsyncActionWithProgress::GetStatus() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_status;
}
...
)
{
    auto threadFunction = [a, f]() mutable
    {
        try
        {
            ...
            if (a->GetStatus() == SpectreAsyncStatus::Started)
            {
                a->SetCompleted();
            }
        }
        catch (Spectre::Framework::CancelledException const&)
        {
            a->SetCanceled();
        }
...
IAsyncActionWithProgress* SPECTRESDK_CALL RunAsAsyncActionWithProgress(
    std::function<void SPECTRESDK_CALL (SF::ICancellationTokenPtr const&,
        ProgressReporter,
        ResourceRequestReporter,
        ProgressIndicatorReporter)> f,
    void* userData,
