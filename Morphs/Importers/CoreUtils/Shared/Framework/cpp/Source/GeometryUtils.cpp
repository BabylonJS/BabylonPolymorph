...
namespace Spectre
{
    namespace Framework
    {
        struct GeometryProcessingException : public Spectre::Utils::SpectreException
        {
            GeometryProcessingException() : SpectreException("Unspecified error while processing geometry", false) {}
            GeometryProcessingException(const std::string& message) : SpectreException(message, false) {}
        };
        ...
        // (Lift viewer project security-testing bug 11228, 11229 and possibly others)
            ...
            Vector3* const normals)
        {
            ...
            for (uint32_t t = 0, i = 0; t < numIndices - boundary; t += offset, ++i)
            {
                ...
                // (Lift viewer project security-testing bug 11233)
            ...
            Vector3* const tan2)
        {
            ...
            for (uint32_t t = 0, i = 0; t < numIndices - boundary; t += offset, ++i)
            {
                ...
                // (Lift viewer project security-testing bug 11227)
                // i0, i1 or i2 may be outside the vertex range, leading to
                // out-of-bounds memory access, hence crash.
                if (i0 >= numVertices || i1 >= numVertices || i2 >= numVertices)
                {
