...
namespace
{
    ...
    std::shared_ptr<const std::string> GetJsonTelemDataString(Babylon::rapidjson::Document& event)
    {
        Babylon::rapidjson::StringBuffer buffer;
        Babylon::rapidjson::Writer<Babylon::rapidjson::StringBuffer> writer(buffer);
        event.Accept(writer);
...
void TelemetryTraceLoggerJson::CanvasInitialization(bool success, const char* exceptionDetails, const char* packageVersion)
{
    ...
    Babylon::rapidjson::Document doc;
    doc.SetObject();
