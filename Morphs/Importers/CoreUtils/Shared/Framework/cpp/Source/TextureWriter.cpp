    ...
    namespace Framework
    {
        namespace V1
        {
                ...
                uint8_t flags)
            {
                if (width >= 65535)
                {
                    throw Utils::BabylonInvalidArgException("Width cannot be greater than 65535");
                }
                if (height >= 65535)
                {
                    throw Utils::BabylonInvalidArgException("Height cannot be greater than 65535");
                }
                if (pixelStride >= 255)
                {
                    throw Utils::BabylonInvalidArgException("PixelStride must be less then 255");
                }
                ...
                uint8_t flags)
            {
                if (width > 0xffff)
                {
                    throw Utils::BabylonInvalidArgException("Width must be less than 65536");
                }
                if (height > 0xffff)
                {
                    throw Utils::BabylonInvalidArgException("Height must be less than 65536");
                }
                if (pixelStride > 255)
                {
                    throw Utils::BabylonInvalidArgException("PixelStride must be less than 256");
                }
                // Write the header...
