...
namespace Spectre
{
    namespace Framework
    {
            ...
            EFrameType frameType)
        {
            ...
            {
                throw Utils::SpectreInvalidArgException("CommonFrameWriter::BeginFrame: No output buffer provided");
            }

            if (m_outputBuffer != nullptr)
            {
                throw Utils::SpectreFailException("CommonFrameWriter::BeginFrame: You must call EndFrame() to close the previous frame, before starting a new frame");
            }

            if (itemCount <= 0)
            {
                throw Utils::SpectreInvalidArgException("CommonFrameWriter::BeginFrame Need at least one item");
            }
...
#error Must be Little or Big endian
        ...
        Frame* CommonFrameWriter::DoEndFrame()
        {
            if (m_outputBuffer == nullptr)
            {
                throw Utils::SpectreFailException("CommonFrameWriter::EndFrame: You must call BeginFrame first");
            }

            if (m_numItems != m_expectedItemCount)
            {
                throw Utils::SpectreFailException("CommonFrameWriter::EndFrame: the number of items added does not match the value specified in BeginFrame()");
            }
            ...
            for (uint32_t i = 0; i < m_savedItemOffsets.size(); ++i)
            {
                ...
                SpectreAssertExpr(offset != 0, "CommonFrameWriter::EndFrame - Offset for item is 0! It cannot be 0");
