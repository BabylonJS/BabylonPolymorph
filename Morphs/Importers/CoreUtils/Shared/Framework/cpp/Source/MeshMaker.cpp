...
//-----------------------------------------------------------------------------
        ...
        void DataMaker::PreprocessMeshes()
        {
            ...
            // to fit the model within the unit area viewing volume. (See MeshMaker::FitMeshToViewingVolume)
            {
                ...
                auto newRootMesh = std::make_shared<MeshMaker>(
                    ...
                    rootID,
                    parentID,
                    [](Babylon::Framework::MeshMaker* maker) { BabylonUnusedParameter(maker); });
            ...
            // The re-center node will be parented to the above root node
            {
                ...
                auto reCenterMesh = std::make_shared<MeshMaker>(
                    ...
                    rootID,
                    parentID,
                    [reCentreTransform](Babylon::Framework::MeshMaker* maker)
                {
        ...
        void MeshMaker::FitMeshToViewingVolume(DirectX::BoundingBox bounds)
        {
            ...
            BabylonAssert(bounds.Extents.x != 0);
            BabylonAssert(bounds.Extents.y != 0);
            BabylonAssert(bounds.Extents.z != 0);
            ...
            uint32_t id)
        {
            ...
            if (it != m_nameMap.end())
            {
                ...
                BabylonAssertExpr(false, "DataMaker::RegisterName: Name and ID clash");
                return;
            }

            m_nameMap[id] = std::move(nameStr);
#else
            BabylonUnusedParameter(nameStr);
            BabylonUnusedParameter(id);
#endif
        }
        ...
        //-----------------------------------------------------------------------------
        void DataMaker::Emit(OnFrameCallback callback, Babylon::Framework::ICancellationTokenPtr cancellationToken)
        {
            if (cancellationToken == nullptr)
            {
                cancellationToken = Babylon::Framework::MakeCancellationToken();
            }

            std::lock_guard<std::mutex> lock(m_lock);

            BabylonAssertExpr(callback != nullptr, "DataMaker::Emit - callback not set");
            ...
            // Output Scene frame next -- this is currently unconditional (always written)
            {
                ...
                catch (const Utils::BabylonException&)
                {
            ...
            m_meshAnalysisResults(meshAnalysisResults)
        {
            ...
            {
                m_parentId = Babylon::Utils::Hash(parentName);
                m_parentName = parentName;
            }
            if (materialName != nullptr)
            {
                m_materialId = Babylon::Utils::Hash(materialName);
        ...
        MeshMaker::MeshMaker(
            ...
            bool areIndices32bit,
            uint32_t formatMask,
            Babylon::Framework::GeometryUtils::TriangleMeshAnalysisResults meshAnalysisResults,
            ProcessMeshCallback callback)
            :
            m_name(name),
            m_id(id),
        ...
        MeshMaker::MeshMaker(
            ...
            :
            m_name(pName),
            m_id(Babylon::Utils::Hash(pName)),
            m_meshAnalysisResults(0)
        {
            if (pParentName != nullptr)
            {
                m_parentName = pParentName;
                m_parentId = Babylon::Utils::Hash(pParentName);
            }
            m_callbacks.push_back(callback);
        }
        ...
        {
            BabylonAssertExpr(m_name.length() > 0, "MeshMaker::CreateMesh - No mesh name provided");
        ...
        {
            BabylonAssert(m_numVertices == 0);
            BabylonAssert(m_numIndices == 0);
        ...
        void MaterialMaker::CreateMaterial(IMaterialWriter* writer)
        {
            ...
            writer->SetBabylonRenderingMode(m_BabylonRenderingMode);
            writer->SetBabylonRenderingOptions(m_BabylonRenderingOptions);
        ...
        TextureMaker::TextureMaker(
            ...
            m_scaleFactor(1.0f)
        {
            BabylonUnusedParameter(reserved);
        }
