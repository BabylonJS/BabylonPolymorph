...
public: // ICancellationToken
    ...
    virtual int SetCancelledCallback(std::function<void SPECTRESDK_CALL (void)> func) override;
...
int SimpleCancellationToken::SetCancelledCallback(std::function<void SPECTRESDK_CALL (void)> func)
{
...
public: // ICancellationToken
    ...
    virtual int AddSlaveToken(ICancellationTokenPtr const& slaveToken) override
    {
        SpectreUnusedParameter(slaveToken);
        return 0;
    }
    ...
    virtual int SetCancelledCallback(std::function<void SPECTRESDK_CALL(void)> func) override
    {
        SpectreUnusedParameter(func);
        return 0;
    }

    virtual void ClearCancelledCallback(int registrationToken) override
    {
        SpectreUnusedParameter(registrationToken);
    }
};
