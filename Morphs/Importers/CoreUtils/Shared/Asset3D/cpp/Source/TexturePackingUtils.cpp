...
namespace
{
    ...
    public:
        PackerDefaultNormalAccessor() :
            m_value(Spectre::Utils::Math::Color(0.5f, 0.5f, 0.5f, 1.0f).BGRA())
        {
        }
    ...
    public:
        PackerDefaultMetalRoughAccessor() :
            m_value(Spectre::Utils::Math::Color(0.0f, 1.0f, 0.0f, 1.0f).BGRA())
        {
        }
    ...
    {
    public:
        PackerTextureAccessor(Spectre::Transcoder::BGRAImage&& image) :
    ...
    )
    {
        ...
        return std::make_shared<TextureDescriptor>(std::move(textureData.m_data),
            textureData.m_dataSize, width, height, width * 4U, Spectre::Framework::TextureSegment::EFormat::kFormatBGRA32);
    }
