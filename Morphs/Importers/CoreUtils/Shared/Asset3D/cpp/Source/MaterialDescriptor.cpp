...
namespace
{
    using TextureFlags = Spectre::Framework::TextureSegment::EFlag;
    ...
    RenderingMode DetectRenderingModeFromDiffuse(const MaterialLayer* const diffuseLayer)
    {
        if (diffuseLayer)
        {
            ...
            if (diffuseLayer->Texture)
            {
                ...
                Spectre::ImagingV2::ImagingComponent::ProcessAlphaFlagsForBGRAImage(diffuseLayer->Texture->GetData(), diffuseLayer->Texture->GetSizeInBytes(), diffuseBgra.Data.data(), diffuseBgra.Width, diffuseBgra.Height, diffuseBgra.RowPitch, textureFlags);
            }
    ...
    RenderingMode DetectRenderingMode(RenderingMode renderingMode, const MaterialLayer* const diffuseLayer)
    {
        ...
        default:
            throw Spectre::Utils::SpectreException("Unrecognized RenderingMode");
        }
    }
    ...
    std::shared_ptr<TextureDescriptor> GenerateLegacyShaderGlossinessChannelIfNeeded(const std::shared_ptr<TextureDescriptor>& sgTexture)
    {
        ...
        Spectre::Transcoder::BGRAImage bgra = { std::vector<uint8_t>(0), 0,0,0 };

        if (sgTexture->GetFlags() == 0)
        {
            uint8_t imageAlphaFlags = 0;
            bgra = sgTexture->EncodeToBGRA();
            Spectre::ImagingV2::ImagingComponent::ProcessAlphaFlagsForBGRAImage(sgTexture->GetData(), sgTexture->GetSizeInBytes(), bgra.Data.data(), bgra.Width, bgra.Height, bgra.RowPitch, imageAlphaFlags);
        ...
        if ((sgTexture->GetFlags() & TextureFlags::kFlag_TextureHasActiveAlpha) != TextureFlags::kFlag_TextureHasActiveAlpha)
        {
            ...
            uint8_t flags = static_cast<uint8_t>(
                Spectre::Framework::TextureSegment::EFlag::kFlag_AlphaValid |
                Spectre::Framework::TextureSegment::EFlag::kFlag_TextureHasActiveAlpha);
...
std::map<LayerType, Spectre::Utils::Math::Color> MaterialDescriptor::GetColors() const
{
