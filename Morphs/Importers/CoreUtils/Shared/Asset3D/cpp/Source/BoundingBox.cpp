...
BoundingBox::BoundingBox(std::vector<Spectre::Utils::Math::Vector3> const& positions)
...
void BoundingBox::Transform(Spectre::Utils::Math::Matrix transform)
{
    if (!m_isInitialised)
    {
        return;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        Update(Spectre::Utils::Math::Vector3::Transform(position, transform));
    }
}
