...
using namespace Spectre::ImagingV2;
        ...
        case TextureContainer::PNG:
            ...
            case TextureEncoding::kPNG256:
                return ImagingComponent::EncodeAsPNG256(
                    ...
                    false);
            default:
                throw Spectre::Utils::SpectreException("Invalid TextureEncoding for PNG container");
            }
...
TextureDescriptor::TextureDescriptor(std::unique_ptr<uint8_t[]>&& data, size_t size)
    : ITraversableWithName(),
      ...
      m_format(Spectre::Framework::TextureSegment::EFormat::kUnknown),
      m_flags(0),
...
TextureDescriptor::TextureDescriptor(std::unique_ptr<uint8_t[]>&& data, size_t size,
        uint32_t width, uint32_t height, uint32_t lineStrideInBytes, Spectre::Framework::TextureSegment::EFormat format, uint8_t flags)
    : ITraversableWithName(),
    m_data(std::move(data)),
    m_sizeInBytes(static_cast<uint32_t>(size)),
    m_height(height),
...
std::vector<uint8_t> Spectre::Transcoder::TextureDescriptor::EncodeTo(TextureContainer container, TextureEncoding encoding, uint32_t width, uint32_t height) const
{
...
std::vector<uint8_t> Spectre::Transcoder::TextureDescriptor::EncodeTo(TextureContainer container, TextureEncoding encoding) const
{
    return ::EncodeTo(container, encoding, m_format, m_data.get(), m_sizeInBytes, m_width, m_height, m_lineStrideInBytes);
}
...
{
    // Fix 15533419:  Make sure we apply the normal map processing that used to occur on the BXF path.
    ...
    bool modified = Spectre::ImagingV2::ImagingComponent::ConvertGrayscaleNormalMap(image, 1.0f);
