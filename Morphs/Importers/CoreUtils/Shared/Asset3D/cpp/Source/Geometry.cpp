...
void Geometry::SetJoints(std::vector<uint32_t>&& jointIndices, std::vector<uint32_t>&& jointWeights)
{
    ...
    {
        throw Utils::SpectreException("Must have the same number of joint indices and weights.");
    }
...
{
    SpectreAssert(morphPositions.size() % 3 == 0);
    SpectreAssert(m_morphTargetPositions.size() == 0 ||
...
{
    SpectreAssert(morphNormals.size() % 3 == 0);
    SpectreAssert(m_morphTargetNormals.size() == 0 ||
...
{
    SpectreAssert(morphTangents.size() % 3 == 0);
    SpectreAssert(m_morphTargetTangents.size() == 0 ||
...
BoundingBox Geometry::CalcTopDownBounds(const Spectre::Utils::Math::Matrix& transform) const
{
