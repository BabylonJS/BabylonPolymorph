...
namespace
{
        ...
        const size_t height)
    {
        ...
        Spectre::Utils::ThrowIfFailed(CanvasTex::Internal::GetImagingFactory(&pFactory));
        ...
        Spectre::Utils::ThrowIfFailed(pFactory->CreatePalette(&thePalette));
        ...
        Spectre::Utils::ThrowIfFailed(thePalette->InitializeCustom(pColors.get(), static_cast<uint32_t>(colorPalette.size())));
        pDest->SetPalette(thePalette.Get());

        WICRect rect = { 0,0, (INT)width, (INT)height };
        Microsoft::WRL::ComPtr<IWICBitmapLock> targetData;
        Spectre::Utils::ThrowIfFailed(pDest->Lock(&rect, WICBitmapLockWrite, &targetData));

        UINT bufSize;
        UINT strideDest;
        BYTE *pRowDest;
        Spectre::Utils::ThrowIfFailed(targetData->GetDataPointer(&bufSize, &pRowDest));
        Spectre::Utils::ThrowIfFailed(targetData->GetStride(&strideDest));
...
void CanvasTex::MedianCutQuantizer::QuantizeImage(const uint8_t* const sourceData, const size_t width, const size_t height, const size_t stride, IWICBitmap* const pDest, const uint32_t nMaxColors)
{
    ...
    SpectreAssert(nMaxColors >= 2);
    SpectreAssert(nMaxColors <= 256);
