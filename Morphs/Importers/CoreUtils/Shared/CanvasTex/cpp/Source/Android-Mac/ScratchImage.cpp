...
// //---------------------------------------------------------------------------------------------------------------------
 ...
 ConstImage ScratchImage::GetImage(size_t item, size_t mip) const
 {
     ...
     {
         throw Babylon::Utils::BabylonException(std::string("ScratchImage::GetImage - Does not support arrays of cubemaps"));
     }
     ...
     case gli::TARGET_2D:
         {
             if (item != 0)
             {
                 throw Babylon::Utils::BabylonException(std::string("ScratchImage::GetImage - Non-zero 'item' for non-array texture"));
             }
     ...
     case gli::TARGET_3D:
         throw Babylon::Utils::BabylonException("ScratchImage::GetImage - Not implemented for 3D textures");
         break;

     default:
         throw Babylon::Utils::BabylonException(std::string("ScratchImage::GetImage - Unknown texture target ") + std::to_string(m_impl->m_texture.target()));
     }
...
size_t ScratchImage::GetImageCount() const
{
    ...
    {
        throw Babylon::Utils::BabylonException(std::string("ScratchImage::GetImageCount - Does not support arrays of cubemaps"));
    }
