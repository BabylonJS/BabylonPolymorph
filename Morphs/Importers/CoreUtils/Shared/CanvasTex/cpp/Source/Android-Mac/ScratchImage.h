...
public:
    ...
    void InitializeFromImage(const ImageBase::ImageImplementation& imgImpl)
    {
        ...
        {
            throw Babylon::Utils::BabylonException(std::string("ScratchImage::InitializeFromImage - Invalid texture format") + std::to_string(std::underlying_type_t<TextureFormat>(imgImpl.format)));
        }
