...
namespace Babylon
{
    namespace Transcoder
    {
        namespace
        {
            void SetTelemetryAnimationTypes(Babylon::Utils::TelemetryTraceLogger::AnimationStats& animStats, Babylon::Utils::glTF::AnimationMetadata::eType animationTypes)
            {
                unsigned int animationType = static_cast<unsigned int>(animationTypes);
                if (animationType & static_cast<unsigned int>(Babylon::Utils::glTF::AnimationMetadata::eType::kMorph))
                {
                    animStats.MarkAnimationType(Babylon::Utils::TelemetryTraceLogger::AnimationStats::AnimationType::Morph);
                }
                if (animationType & static_cast<unsigned int>(Babylon::Utils::glTF::AnimationMetadata::eType::kNode))
                {
                    animStats.MarkAnimationType(Babylon::Utils::TelemetryTraceLogger::AnimationStats::AnimationType::Node);
                }
                if (animationType & static_cast<unsigned int>(Babylon::Utils::glTF::AnimationMetadata::eType::kSkinned))
                {
                    animStats.MarkAnimationType(Babylon::Utils::TelemetryTraceLogger::AnimationStats::AnimationType::Skeletal);
                }
            }

            void GetAnimationStats(Babylon::Utils::TelemetryTraceLogger::AnimationStats& animStats, const Asset3DPtr& asset3D)
            {
                ...
                {
                    BabylonAssert(asset3D->GetSkeletons().empty());
                    BabylonAssert(asset3D->GetAnimations().empty());
        ...
        void TranscodersTelemetryHelper::Initialize(const std::unordered_map<std::string, std::string>& transcodeOptions, Babylon::Utils::TelemetryTraceLoggerFactoryFunctionType factory)
        {
            ...
            if (factory == nullptr)
            {
                factory = Babylon::Utils::CreateTelemetryTraceLogger;
            }
        ...
        void TranscodersTelemetryHelper::SendImportCompleteTelemetry(const std::shared_ptr<Babylon::Transcoder::Asset3D> asset3DFromImport, const uint64_t streamByteSize)
        {
            if (!m_telemetryEnabled)
            {
                return;
        ...
        void TranscodersTelemetryHelper::SendExportCompleteTelemetry(const std::shared_ptr<Babylon::Transcoder::Asset3D> asset3DFromImport)
        {
            if (!m_telemetryEnabled)
            {
                return;
        ...
        void TranscodersTelemetryHelper::SendTranscoderInitialization()
        {
            ...
            // TODO: TEMP COMMENT
            //m_telemetryLogger->TranscoderInitialization(true, "", Babylon::Transcoder::GetVersionString(), m_transcoderVersion.c_str());
        }
