...
namespace
{
    ...
    T GetElementFromObjStringIndex(const std::vector<T>& collection, const std::string& indexStr)
    {
        if (indexStr.empty())
        {
            throw Babylon::Utils::BabylonException("Invalid index: empty");
        }
        ...
        if (index >= collection.size())
        {
            ...
            throw Babylon::Utils::BabylonException(errorMsg.str());
        }

        return collection[index];
    }

    Babylon::Utils::Math::Vector3 FlipYZ(const Babylon::Utils::Math::Vector3& vec3)
    {
        return Babylon::Utils::Math::Vector3(vec3.x, vec3.z, -vec3.y);
    }
}
...
GeometryParser::ObjVertex GeometryParser::GetVertex(const std::string& vertexStr) const
{
    ...
    auto split = Babylon::Utils::Split(vertexStr, kVertexDelimiter);
    if (split.size() > 0)
    {
        objVertex.v = split[0];
        if (split.size() > 1)
...
void GeometryParser::ProcessSmoothingGroup(LineTokensPtr& tokens)
{
    ...
    if (tokens->size() < 2)
    {
        throw Babylon::Utils::BabylonException("Bad smoothing group. Expected a token after 's'.");
    }
...
void GeometryParser::ProcessComment(LineTokensPtr& tokens)
{
        ...
        m_author == unknownAuthor)
    {
        ...
        for (size_t i = 1; i < tokens->size(); ++i)
        {
            ...
            Babylon::Utils::TransformToLower(token);
