...
namespace
{
    ...
    bool IsExtrasAttributeSetAndTrue(const std::string& extrasJson, const char* attributeName)
    {
        ...
        Babylon::rapidjson::Document extrasDoc;
    ...
    void CopyFromGltfProperty(const glTF::glTFProperty& gltfProperty, ITraversableBase* a3dNode, const std::string& key = Babylon::Utils::glTF::kExtensionsDefault,
    ...
    void CopyFromGltfChildOfRootProperty(const glTF::glTFChildOfRootProperty& gltfProperty, ITraversableWithName* a3dNode, const std::string& key = Babylon::Utils::glTF::kExtensionsDefault,
        const std::unordered_set<std::string>& extensionBlacklist = {})
    {
    ...
    std::shared_ptr<Babylon::Transcoder::Camera> GltfCameraToA3dCamera(const Microsoft::glTF::Camera& gltfCamera)
    {
        std::shared_ptr<Babylon::Transcoder::Camera> a3dCamera;
        if (gltfCamera.projection->GetProjectionType() == Microsoft::glTF::PROJECTION_ORTHOGRAPHIC)
        {
            ...
            a3dCamera = std::make_shared<Babylon::Transcoder::Camera>(
                std::make_unique<Babylon::Transcoder::Orthographic>(orthographic.zfar, orthographic.znear, orthographic.xmag, orthographic.ymag));
        }
        else if (gltfCamera.projection->GetProjectionType() == Microsoft::glTF::PROJECTION_PERSPECTIVE)
        {
            ...
            auto a3dPerspective = std::make_unique<Babylon::Transcoder::Perspective>(perspective.znear, perspective.yfov);
            ...
            a3dCamera = std::make_shared<Babylon::Transcoder::Camera>(std::move(a3dPerspective));
        }
    ...
    std::shared_ptr<Babylon::Transcoder::Mesh> GltfMeshToA3dMesh(const Microsoft::glTF::Mesh& mesh)
    {
        auto a3dMesh = std::make_shared<Mesh>();
        a3dMesh->SetMorphWeights(mesh.weights);

        bool isMinecraftModel = IsExtrasAttributeSetAndTrue(mesh.extras, Babylon::Utils::glTF::kMinecraftExtrasAttributeName);
        if (isMinecraftModel)
        {
    ...
    std::vector<uint32_t> GetTruncatedJointIndices32(const glTF::Document& doc, const glTF::GLTFResourceReader& reader, const glTF::Accessor& accessor)
    {
        ...
        for (unsigned int i = 0; i < joints.size(); i += 4)
        {
            ...
            {
                throw Babylon::Utils::BabylonException("16-bit joint indices not supported");
            }
...
std::unique_ptr<Asset3D> GLTFToAsset3DConverter::CreateAsset3D(const Microsoft::glTF::Document& gltfDocument, Babylon::Framework::ICancellationTokenPtr cancellationToken)
{
    ...
    if (version.major != 2U)
    {
        throw Utils::BabylonException("glTF document version is not supported");
    }

    // Minimum version check
    if (!glTF::IsMinVersionRequirementSatisfied(gltfDocument.asset.minVersion))
    {
        throw Utils::BabylonException("glTF document minimum version is greater than 2.0");
    }
    ...
    bool isLegacyGLTF = Babylon::Utils::glTF::IsLegacyGLTFDocument(gltfDocument);
    ...
    CopyFromGltfProperty(gltfDocument, asset3d.get(), Babylon::Utils::glTF::kExtensionsRootGltfKey);

    // Add extensions from root scene node
    CopyFromGltfChildOfRootProperty(scene, asset3d.get(), Babylon::Utils::glTF::kExtensionsDefaultSceneKey);
    ...
    if (gltfDocument.animations.Elements().size() > 0 || gltfDocument.skins.Elements().size() > 0)
    {
        ...
        auto buildRemappingMap = [&visitorStateNodes](const auto& targetedNodeIds)
        {
            ...
            for (auto& nodeId : targetedNodeIds)
            {
                ...
                {
                    throw Utils::BabylonException("Animation target node not found in scene");
                }
            }
    ...
    Babylon::Framework::ICancellationTokenPtr cancellationToken)
{
    ...
    switch (primitive.mode)
    {
        ...
        default:
            throw Utils::BabylonException("Unrecognized primitive mode.");
    }
...
std::shared_ptr<MaterialDescriptor> GLTFToAsset3DConverter::ParseMaterial(VisitorState& visitorState, const std::string& materialId)
{
    ...
    CopyFromGltfChildOfRootProperty(mat, materialDesc.get(), Babylon::Utils::glTF::kExtensionsDefault, { MSFT_LOD });
...
std::shared_ptr<MaterialLayer::TextureSampler> GLTFToAsset3DConverter::ParseSampler(const Microsoft::glTF::Document& gltfDocument, const std::string& samplerId)
{
    ...
    {
        const auto gltfMagFilter = gltfSampler.magFilter ? gltfSampler.magFilter.Get() : Babylon::Utils::glTF::GetDefaultMagFilterMode();
        const auto gltfMinFilter = gltfSampler.minFilter ? gltfSampler.minFilter.Get() : Babylon::Utils::glTF::GetDefaultMinFilterMode();
...
void GLTFToAsset3DConverter::PopulateMorphTargetPositions(const Microsoft::glTF::Document& gltfDocument, Geometry& geometry, const Microsoft::glTF::MorphTarget& morphTarget) const
{
    ...
    BabylonUnusedLocal(targetPositionAccessor);
    BabylonAssert(targetPositions.size() % 3 == 0);
    BabylonAssert(targetPositions.size() / 3 == targetPositionAccessor.count);
    if ((targetPositions.size() / 3 != geometry.GetPositions().size()) && (geometry.GetPositions().size() > 0))
    {
        throw Utils::BabylonException("Morph Target position count differs from mesh.");
    }
...
void GLTFToAsset3DConverter::PopulateMorphTargetNormals(const Microsoft::glTF::Document& gltfDocument, Geometry& geometry, const Microsoft::glTF::MorphTarget& morphTarget) const
{
    ...
    BabylonUnusedLocal(targetNormalsAccessor);
    BabylonAssert(targetNormals.size() % 3 == 0);
    BabylonAssert(targetNormals.size() / 3 == targetNormalsAccessor.count);
    if ((targetNormals.size() / 3 != geometry.GetNormals().size()) && (geometry.GetNormals().size() > 0))
    {
        throw Utils::BabylonException("Morph Target normal count differs from mesh.");
    }
...
void GLTFToAsset3DConverter::PopulateMorphTargetTangents(const Microsoft::glTF::Document& gltfDocument, Geometry& geometry, const Microsoft::glTF::MorphTarget& morphTarget) const
{
    ...
    BabylonUnusedLocal(targetTangentsAccessor);
    BabylonAssert(targetTangents.size() % 3 == 0);
    BabylonAssert(targetTangents.size() / 3 == targetTangentsAccessor.count);
    if ((targetTangents.size() / 3 != geometry.GetTangents().size()) && (geometry.GetTangents().size() > 0))
    {
        throw Utils::BabylonException("Morph Target tangent count differs from mesh.");
    }
