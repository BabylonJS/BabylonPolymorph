...
void FileParserBase::ParseStream(std::shared_ptr<TranscoderTextStream> stream, size_t maxLineSize)
{
    ...
    do
    {
        ...
        {
            float percentage = m_streamLength != 0 ? Babylon::Utils::Math::Clamp((float)m_charactersRead / (float)m_streamLength, 0.0f, 1.f) : 0.1f;
...
{
    BabylonUnusedParameter(lineLength);
