...
namespace glTFUtils = Spectre::Utils::glTF;

namespace
{
    void AppendElementValues(std::vector<float>& elements, const Spectre::Utils::Math::Quaternion& element)
    {
    ...
    void AppendElementValues(std::vector<float>& elements, const Spectre::Utils::Math::Vector3& element)
    {
        elements.push_back(element.x);
        elements.push_back(element.y);
        elements.push_back(element.z);
    ...
    PositionExtents GetExtents(const std::vector<Spectre::Utils::Math::Vector3>& positions)
    {
    ...
    void ValidateNormals(const std::vector<Spectre::Utils::Math::Vector3>& normals)
    {
        size_t warnCount = 0;
        for (size_t i = 0; i < normals.size(); i++)
        {
            if (normals[i] == Spectre::Utils::Math::Vector3::Zero)
            {
                warnCount++;
                if (warnCount < 10)
                {
    ...
    void ValidateTangents(const std::vector<Spectre::Utils::Math::Vector4>& tangents)
    {
        size_t warnCount = 0;
        for (size_t i = 0; i < tangents.size(); i++)
        {
            if (tangents[i] == Spectre::Utils::Math::Vector4::UnitW)
            {
                warnCount++;
                if (warnCount < 10)
                {
            ...
            //     auto vec = Spectre::Utils::Math::Vector3(tangent.x, tangent.y, tangent.z);
            //     vec.Normalize();
            //     return Spectre::Utils::Math::Vector4(vec.x, vec.y, vec.z, std::copysign(1.0f, tangent.w));
        };
    ...
    std::vector<float> GetRotationKeyframeValues(const std::vector<Spectre::Transcoder::Animation::MotionState>& jointMotion)
    {
        auto getElementFunction = [&jointMotion](size_t i)
        {
            Spectre::Utils::Math::Quaternion normalized;
    ...
    std::vector<float> GetTranslationKeyframeValues(const std::vector<Spectre::Transcoder::Animation::MotionState>& jointMotion)
    {
        auto getElementFunction = [&jointMotion](size_t i)
        {
    ...
    std::vector<float> GetScaleKeyframeValues(const std::vector<Spectre::Transcoder::Animation::MotionState>& jointMotion)
    {
        auto getElementFunction = [&jointMotion](size_t i)
        {
    ...
    bool MatrixDecompose(const Spectre::Utils::Math::Matrix& matrix,
        Spectre::Utils::Math::Vector3& outScale,
        Spectre::Utils::Math::Quaternion& outRotation,
        Spectre::Utils::Math::Vector3& outTranslation)
    {
        ...
        Spectre::Utils::Math::Vector3 axisX = matrix.Right();
        Spectre::Utils::Math::Vector3 axisY = matrix.Up();
        Spectre::Utils::Math::Vector3 axisZ = matrix.Backward();
        Spectre::Utils::Math::Vector3 translation = matrix.Translation();
        ...
        if (axisX == Spectre::Utils::Math::Vector3::Zero &&
            axisY == Spectre::Utils::Math::Vector3::Zero &&
            axisZ == Spectre::Utils::Math::Vector3::Zero)
        {
            outScale = Spectre::Utils::Math::Vector3::Zero;
            outRotation = Spectre::Utils::Math::Quaternion::Identity;
        ...
        Spectre::Utils::Math::Vector3 scaleExtracted{ lengthX, lengthY, lengthZ };
        ...
        Spectre::Utils::Math::Matrix matrixAdjusted = Spectre::Utils::Math::Matrix
        {
            Spectre::Utils::Math::Vector4::ToVector4(axisX, 0.0f),
            Spectre::Utils::Math::Vector4::ToVector4(axisY, 0.0f),
            Spectre::Utils::Math::Vector4::ToVector4(axisZ, 0.0f),
            Spectre::Utils::Math::Vector4::ToVector4(translation, 1.0f)
        };
    ...
    void SetGltfTRS(glTF::Node& node, const Spectre::Utils::Math::Matrix& sourceMatrix)
    {
        Spectre::Utils::Math::Vector3 translation;
        Spectre::Utils::Math::Quaternion rotation;
    ...
    std::string GetGenerator()
    {
        std::stringstream ss;
        // TODO: TEMP COMMENT
        ss << glTF::MSFT_GLTF_EXPORTER_NAME << " "; // << SPECTRE_PACKAGE_VERSION;
        return ss.str();
    }
    ...
    void CopyToGltfProperty(const ITraversableBase& a3dNode, glTF::Document& doc, glTF::glTFProperty& gltfNode, const std::string& extensionsKey = Spectre::Utils::glTF::kExtensionsDefault)
    {
        SpectreAssert(gltfNode.extras.empty());
        CopyExtensionsToNode(a3dNode, doc, gltfNode, extensionsKey);
        CopyExtrasToNode(a3dNode, gltfNode);
    }

    Microsoft::glTF::MeshMode GetGltfMeshPrimitiveMode(const Spectre::Transcoder::GeometryTopology topology)
    {
        switch (topology)
        {
        ...
        throw Spectre::Utils::SpectreException("Unrecognized primitive mode.");
    }

    glTF::Camera ToGLTFCamera(const Spectre::Transcoder::Camera& cam)
    {
        switch (cam.GetProjectionType())
        {
        case Spectre::Transcoder::ProjectionType::kPerspective:
        {
        ...
        case Spectre::Transcoder::ProjectionType::kOrthographic:
        {
        ...
        default:
            throw Spectre::Utils::SpectreException("Unrecognized camera projection type");
        }
    }

    glTF::Color3 ToGLTFColor3(const Spectre::Utils::Math::Color& color)
    {
        return glTF::Color3(color.R(), color.G(), color.B());
    }

    glTF::Color4 ToGLTFColor4(const Spectre::Utils::Math::Color& color)
    {
    ...
    glTF::AlphaMode ToGLTFAlphaMode(RenderingMode renderingMode)
    {
        ...
        default:
            throw Spectre::Utils::SpectreException("Call MaterialDescriptor::ConvertFromLegacy() to detect exact RenderingMode");
        }
    }
}

Asset3DToGLTFConverter::Asset3DToGLTFConverter(const Asset3D& asset3d, const GLTFExportOptions& options, Spectre::Framework::ICancellationTokenPtr cancellationToken)
    : m_asset3d(asset3d),
    m_options(options),
...
glTF::MeshPrimitive Asset3DToGLTFConverter::CreateMeshPrimitive(glTF::BufferBuilder& bufferBuilder, const Geometry& geometry) const
{
    ...
    if (geometry.HasMorphAnimation())
    {
        ...
        SpectreAssert(morphTargetsNormals.size() == 0 || (morphTargetsNormals.size() == morphTargetsPositions.size()));
        ...
        SpectreAssert(morphTargetsTangents.size() == 0 || (morphTargetsTangents.size() == morphTargetsPositions.size()));
...
void Asset3DToGLTFConverter::PopulateDocument(IGLTFWriter& writer)
{
    ...
    CopyToGltfProperty(m_asset3d, *m_gltfDocument, *m_gltfDocument.get(), Spectre::Utils::glTF::kExtensionsRootGltfKey);

    glTF::Scene gltfScene;
    gltfScene.id = m_asset3d.GetIdString();

    CopyToGltfProperty(m_asset3d, *m_gltfDocument, gltfScene, Spectre::Utils::glTF::kExtensionsDefaultSceneKey);
...
TextureCompressionType Asset3DToGLTFConverter::GetCompliantTextureCompressionType(TextureChannels channels)
{
    ...
    default:
        throw Spectre::Utils::SpectreException("Unrecognized ExportTextureMode");
    }
}
...
TextureCompressionType Asset3DToGLTFConverter::GetDDSTextureCompressionType(TextureChannels channels)
{
    ...
    default:
        throw Spectre::Utils::SpectreException("Unrecognized TextureChannels");
    }
}
...
void Asset3DToGLTFConverter::SetTextureInfo(const std::shared_ptr<MaterialDescriptor::LayerInfo>& layer, glTF::TextureInfo& textureInfo, IGLTFWriter& writer, const uint8_t scale, TextureChannels channels, bool compliantImage, bool ddsImage)
{
    if (layer->Texture)
    {
        ...
        if (layer->TextureTransform != Spectre::Utils::Math::Matrix::Identity)
        {
            bool hasTextureTransform = Spectre::Utils::glTF::TryGetTextureTransform(layer->TextureTransform, textureInfo);
...
{
    if (layer && layer->TextureTransform != Spectre::Utils::Math::Matrix::Identity)
    {
        bool hasTextureTransform = Spectre::Utils::glTF::TryGetTextureTransform(layer->TextureTransform, textureInfo);
        if (hasTextureTransform)
        {
...
glTF::Material Asset3DToGLTFConverter::AddMaterial(IGLTFWriter& writer, MaterialDescriptor* materialDesc, const uint8_t scale)
{
    ...
    default:
        throw Spectre::Utils::SpectreException("Unrecognized TranscodingMode");
    }
...
std::string Asset3DToGLTFConverter::AddSampler(const MaterialLayer::TextureSampler& sampler)
{
    ...
    if (sampler.filter)
    {
        using EFilterMode = Spectre::Framework::SamplerDefinition::EFilterMode;
        ...
        // Note that Spectre and D3D11 don't have the notion of a sampler
    ...
    using EAddressMode = Spectre::Framework::SamplerDefinition::EAddressMode;
...
{
    SpectreAssert(compliantImage || ddsImage);
    ...
    if (compliantImage)
    {
        ...
        case TextureCompressionType::JPEG:
            ...
            break;
        default:
            throw Spectre::Utils::SpectreException("Unsupported CompressionType");
        }
