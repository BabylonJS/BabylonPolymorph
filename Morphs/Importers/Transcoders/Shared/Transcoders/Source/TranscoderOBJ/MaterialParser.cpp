...
{
    BabylonAssert(tokens->size() > 0);
    BabylonAssert((*tokens)[0].size() > 0);
...
void MaterialObjParser::AddNewMaterial(LineTokensPtr& tokens)
{
    if (tokens->size() != 2)
    {
        throw Babylon::Utils::BabylonException("Expected 2 tokens for the material name.");
    }
...
void MaterialObjParser::Update(LineTokensPtr& tokens)
{
    ...
    std::shared_ptr<Babylon::Transcoder::TextureDescriptor> texture;
    switch (lineType)
    {
    ...
    {
        auto smoothness = Babylon::Utils::Math::Clamp(SpecularPowerToSmoothness(m_tokenParser.GetFloatAtPosition(tokens, 1)), 0.0f, 1.0f);
        auto specularLayer = m_currentMaterial->GetOrAddLayer(LayerType::kSpecularGlossiness);
        if (!specularLayer->Color)
        {
...
Babylon::Utils::Math::Color MaterialObjParser::CreateColor(LineTokensPtr& tokens) const
{
    ...
    if (tokens->size() != 4)
    {
        ...
        throw Babylon::Utils::BabylonException("Unexpected number of tokens while parsing color.");
    }

    // In the form "Kd spectral file.rfl factor"
    if ((*tokens)[1] == "spectral")
    {
        throw Babylon::Utils::BabylonException("Not supported color mode.");
    }
