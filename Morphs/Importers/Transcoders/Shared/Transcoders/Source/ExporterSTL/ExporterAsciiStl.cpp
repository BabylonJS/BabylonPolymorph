...
void ExporterAsciiStl::ExportStatic(Asset3DPtr asset3D, const std::string& assetName, IOutputStreamFactory* streamFactory, FractionalProgressCallback progress, Babylon::Framework::ICancellationTokenPtr cancellationToken, const std::unordered_map<std::string, std::string>& options)
{
...
ExporterAsciiStl::ExporterAsciiStl(std::string assetName, std::ostream& outputStream, Babylon::Framework::ICancellationTokenPtr cancellationToken)
...
{
    BabylonUnusedParameter(asset3D);
...
{
    BabylonUnusedParameter(sceneNode);
    m_transforms.pop();
}
