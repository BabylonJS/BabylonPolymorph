...
namespace
{
    ...
    inline int64_t StrToInt64(const std::string& str)
    {
        ...
        if (read != str.length())
        {
            throw SpectreException("Failed to parse all characters of string.");
        }
    ...
    namespace Transcoder
    {
        ...
        void PLYParser::ComputeNormals()
        {
            if (m_vertices.size() > 0)
            {
                ...
                Spectre::Framework::GeometryUtils::ComputeNormals(vertexs, outputVertexCount, indices, outputIndexCount, d);
            }
        }
        ...
        void PLYParser::AddFileType(const std::vector<std::string>& tokens)
        {
            ...
            if (tokens.size() > 1)
            {
                ...
                if (auto telemetryManager = Spectre::Utils::TelemetrySingleton::get())
                {
                    StringBuffer buffer;
        ...
        void PLYParser::ParseStreamInternal()
        {
            if (m_streamLength <= PLYMinimumValidLength)
            {
                ...
                throw Utils::SpectreException(errorMsg);
            }

            if (!ParseHeader())
            {
                throw Utils::SpectreException("Invalid Header");
            }
            ...
            {
                throw Utils::SpectreException("PLY files with vertices but no indices are not supported");
            }
                ...
                if (element.name == "vertex")
                {
                    ...
                    for (int i = 0; i < element.num; i++)
                    {
                        ...
                        GetElement(element, [&vert, &uv](
                            ...
                            float float_value)
                        {
                            SpectreUnusedParameter(uint_value);
                            SpectreUnusedParameter(int_value);
                    ...
                    for (int i = 0; i < element.num; i++)
                    {
                            ...
                            float float_value)
                        {
                            if (name == "vertex_indices" ||
                                name == "vertex_index")
                            {
                                SpectreUnusedParameter(index);
                                SpectreUnusedParameter(int_value);
                                SpectreUnusedParameter(float_value);
