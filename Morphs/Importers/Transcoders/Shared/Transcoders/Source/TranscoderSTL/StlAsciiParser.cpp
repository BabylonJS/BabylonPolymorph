...
namespace
{
    Spectre::Transcoder::LineTypeTranslator<StlAsciiParser::LineType> GeometryLineTypeTranslator()
    {
...
const LineTypeTranslator<StlAsciiParser::LineType> Spectre::Transcoder::StlAsciiParser::m_lineTypeTranslator = GeometryLineTypeTranslator();

StlAsciiParser::StlAsciiParser(IResourceServer& resourceServer, UpdateReporter* updateReporter, Spectre::Framework::ICancellationTokenPtr cancellationToken)
...
//   endfacet
// endsolid [name]
void StlAsciiParser::ProcessNonEmptyLine(Spectre::Transcoder::LineTokensPtr& tokens)
{
    ...
    default:
        throw Spectre::Utils::SpectreException("Unexpected element type: " + tokens->at(0));
    }
}
...
void StlAsciiParser::CheckAndPush(LineType current, LineType parent, const std::string& message)
{
    ...
    {
        throw Spectre::Utils::SpectreException(message);
    }

    m_hierarchy.push(current);
}

void StlAsciiParser::CheckAndPop(LineType sibling, const std::string& message, const Spectre::Transcoder::LineTokensPtr& tokens)
{
...
void StlAsciiParser::ParseSolid(const Spectre::Transcoder::LineTokensPtr& tokens)
{
    if (tokens->size() == 2)
    {
    ...
    {
        throw Spectre::Utils::SpectreException("Unexpected 'solid' element found inside another element.");
    }

    m_hierarchy.push(LineType::Solid);
}

void StlAsciiParser::ParseFacet(const Spectre::Transcoder::LineTokensPtr& tokens)
{
...
void StlAsciiParser::ParseOuterLoop(const Spectre::Transcoder::LineTokensPtr& tokens)
{
    ...
    if (tokens->at(1) != "loop")
    {
        throw Spectre::Utils::SpectreException("Expected 'loop' after 'outer' element.");
    }
}

void StlAsciiParser::ParseVertex(const Spectre::Transcoder::LineTokensPtr& tokens)
{
    if (tokens->size() != 4)
    {
        throw Spectre::Utils::SpectreException("Unexpected number of tokens for 'vertex' element: %zu", tokens->size());
    }

    if (m_hierarchy.top() != LineType::OuterLoop)
    {
        throw Spectre::Utils::SpectreException("Expected 'vertex' element to be inside 'outer loop' element.");
    }
...
void StlAsciiParser::ParseEndLoop(const Spectre::Transcoder::LineTokensPtr& tokens)
{
    CheckAndPop(LineType::OuterLoop, "Expected 'endloop' element at the same level as 'loop' element.", tokens);

    if (m_positions.size() % 3 != 0)
    {
        throw Spectre::Utils::SpectreException("Expected multiple of 3 vertices.");
    }
}
...
void StlAsciiParser::FinalizeParseStream()
{
    if (!m_hierarchy.empty())
    {
        throw Spectre::Utils::SpectreException("Unexpected end of file.");
    }
