...
namespace Spectre
{
    namespace Transcoder
    {
        ...
        // TODO: TEMP COMMENT
        //const char* SPECTRESDK_CALL GetVersionString();
        ...
        /// \return                      An Asset3D
        using ImportFunction = std::function<Asset3DPtr SPECTRESDK_CALL (
        ...
        using ExportFunction = std::function<void SPECTRESDK_CALL(
            Asset3DPtr asset3D,
        ...
        void SPECTRESDK_CALL RegisterImportFunction(const std::string& extension, ImportFunction fn);

        /// \brief
        /// Register an export function for a specific file extension type.
        /// \param extension             File type (e.g. "gltf", "obj")
        /// \param fn                    Function pointer to register
        void SPECTRESDK_CALL RegisterExportFunction(const std::string& extension, ExportFunction fn);

        /// \brief
        /// Register an import help function for a specific file extension type.
        /// \param extension             File type (e.g. "gltf", "obj")
        /// \param fn                    Function pointer to register
        void SPECTRESDK_CALL RegisterImportHelpFunction(const std::string& extension, HelpFunction fn);

        /// \brief
        /// Register an export help function for a specific file extension type.
        /// \param extension             File type (e.g. "gltf", "obj")
        /// \param fn                    Function pointer to register
        void SPECTRESDK_CALL RegisterExportHelpFunction(const std::string& extension, HelpFunction fn);

        /// \brief
        /// Return if there is an importer registered for a given extension
        bool SPECTRESDK_CALL HasImporter(const std::string& extension);

        /// \brief
        /// Return if there is an exporter registered for a given extension
        bool SPECTRESDK_CALL HasExporter(const std::string& extension);
        ...
        std::set<std::string> SPECTRESDK_CALL GetImportFormats();

        /// \brief
        /// Return a vector of all supported export formats
        std::set<std::string> SPECTRESDK_CALL GetExportFormats();
        ...
        /// \return                      An Asset3D
        Asset3DPtr SPECTRESDK_CALL Import(
            const std::string& importFormat,
            const std::string& importFilename,
            IResourceServer* importResourceServer,
            FractionalProgressCallback progress = nullptr,
            Spectre::Framework::ICancellationTokenPtr cancellationToken = nullptr,
            const char* jsonConfig = nullptr,
            Spectre::Utils::TelemetryTraceLoggerFactoryFunctionType factory = nullptr);
